            ...
            @foreach ($landscapeMedia->sortByDesc('created_at') as $gallery)
                <div class="relative bg-gray-200 shadow-lg rounded-lg overflow-hidden">
                    <a href="#" class="open-modal" data-media="{{ Storage::url($gallery->media) }}"
                        data-type="{{ in_array(pathinfo($gallery->media, PATHINFO_EXTENSION), ['jpeg', 'png', 'jpg', 'gif', 'webp', 'PNG', 'JPG', 'JPEG', 'GIF']) ? 'image' : 'video' }}">
                        @if (in_array(pathinfo($gallery->media, PATHINFO_EXTENSION), ['jpeg', 'png', 'jpg', 'gif', 'webp', 'PNG', 'JPG', 'JPEG', 'GIF']))
